...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    if (req.method === 'GET') {
      ...
      const analysesRes = await db.execute('SELECT id, job_title, overall_score, score_breakdown, suggestions, keywords, formatting_issues, page_optimization, created_at FROM analyses WHERE user_id = ? ORDER BY created_at DESC', [userId]);
